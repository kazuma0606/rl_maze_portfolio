...
7. **EX_algorithm_comparison.ipynb** - アルゴリズム比較分析
   - Random Agent、DQN、PPOの包括的な比較
   - パフォーマンス指標（成功率、報酬、エピソード長）
   - ニューラルネットワークアーキテクチャの可視化
   - 推論パフォーマンスのベンチマーク
...
### 2.1 Graphvizのインストール（アーキテクチャ可視化用・オプション）

アルゴリズム比較ノートブック（`EX_algorithm_comparison.ipynb`）でニューラルネットワークのグラフ可視化を行う場合は、Graphvizが必要です。

#### Windows

1. [Graphviz公式サイト](https://graphviz.org/download/)から最新版をダウンロード
2. インストーラーを実行（推奨: `C:\Program Files\Graphviz`）
3. システム環境変数PATHに追加：
   - `C:\Program Files\Graphviz\bin` または `C:\Program Files (x86)\Graphviz\bin`
4. コマンドプロンプト/PowerShellを再起動
5. 確認：
   ```bash
   where.exe dot
   # 出力: C:\Program Files\Graphviz\bin\dot.exe
   ```
...
```bash
dot -V
# 出力例: dot - graphviz version 2.50.0 (20211204.2007)
```

**注意:** Graphvizがインストールされていない場合でも、ノートブックは正常に動作します。グラフ可視化セクションのみがスキップされます。
...
| torchviz | >=0.0.2 | ニューラルネットワークの可視化 |
| torchsummary | >=1.5.1 | モデルアーキテクチャの要約 |
...
├── EX_algorithm_comparison.ipynb  # アルゴリズム比較分析
├── test_algorithm_comparison.py   # 比較機能のテスト
